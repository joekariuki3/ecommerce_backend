...
class TestSortProductByName:
    ...
    def test_sort_products_by_name_with_identical_names(
        self, api_client, product_factory, category_factory
    ):
        category = category_factory(name="Category 1")
        product_factory(
...
class TestSortProductByPrice:
    def test_sort_products_by_price_ascending(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) == 3
        returned_product_prices = [
            product["price"] for product in response.data["results"]
        ]
    ...
    def test_sort_products_by_price_with_identical_prices(
        self, api_client, product_factory, category_factory
    ):
        category = category_factory(name="Category 1")
        product_factory(
        ...
        assert returned_product_prices == [
            "20.00",
            "20.00",
            "30.00",
            "30.00",
...
class TestSortProductByCreatedAt:
    def test_sort_products_by_created_at_ascending(
        self, api_client, product_factory, category_factory
    ):
        category = category_factory(name="Category 1")
        product_factory(
    ...
    def test_sort_products_by_created_at_descending(
        self, api_client, product_factory, category_factory
    ):
        category = category_factory(name="Category 1")
        product_factory(
